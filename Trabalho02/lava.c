...
#define LIFE_TIME 10000
#define NUM_OF_SMOKE 10000
...
GLfloat init_x = 0, init_y = 0, init_z = 0, max_init_speed = 0.0010, max_y_init_speed = 0.001, gravity = -0.0005; 

particle smoke[NUM_OF_SMOKE];
GLfloat init_smoke_x = 0, init_smoke_y = 0.01, init_smoke_z = 0, max_init_smoke_speed = 0.002,
max_y_init_smoke_speed =  0.1, h_max = 3; 
...
void moveSmoke(){
    ...
    glBegin(GL_POINTS);
        ...
        for(int i = 0; i < NUM_OF_SMOKE; i++){
            ...
            smoke[i].v_x = cos(rad(random_arg))*smoke[i].y/200;        
            smoke[i].v_z = sin(rad(random_arg))*smoke[i].y/200;        
            smoke[i].v_y = smoke[i].v_y/1.00000001;
            smoke[i].lifetime--;    
...
void generateParticle(){
    ...
    for(int i = 0; i < NUM_OF_PARTICLES; i++){
        ...
        lava[i].v_x = (GLfloat) max_init_speed*cos(rad(random_arg))*powf(-1, (GLfloat) rand());        
        lava[i].v_z = (GLfloat) max_init_speed*sin(rad(random_arg))*powf(-1, (GLfloat) rand());        
...
void regenerateParticle(int i){
    ...
    random_arg = (double) rand();
    lava[i].v_x = (GLfloat) max_init_speed*cos(rad(random_arg))*powf(-1, rand()); 
    random_arg = (double) rand();
    lava[i].v_z = (GLfloat) max_init_speed*sin(rad(random_arg))*powf(-1, rand());        
    random_arg = (double) rand();
    lava[i].v_y = (GLfloat) max_y_init_speed*fabs(tan(fmod(rad(random_arg), (M_PI/2)) ));
    ...
    glBegin(GL_POINTS);
        ...
        for(int i = 0; i < NUM_OF_PARTICLES; i++){
            ...
            if(lava[i].y > ground){
                ...
                lava[i].y = maxf(lava[i].y + lava[i].v_y, ground - 0.0001);
                // lava[i].v_x += slower;        
                // lava[i].v_z += slower;        
                lava[i].v_y += gravity;        
            }
